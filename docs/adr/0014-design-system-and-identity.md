...
## Amendment (2026-09-16): deferred pieces realized

Two items this ADR deferred are now built (owner request to fully match the Studio
mock), still without a wire-protocol change:

- **Studio typefaces are bundled.** Bricolage Grotesque (display), IBM Plex Sans
  (body), and IBM Plex Mono are subset to Latin + punctuation and embedded as
  base64 woff2 in the shell (`frontend/src/fonts.css`, regenerated by
  `tools/build-fonts.py`). The shell still fetches nothing at runtime (ADR-0004) —
  the fonts travel inside the single bundled `index.html` (~+125 KB gzip).
- **User-selectable light/dark theme.** A `<head>` script resolves the saved choice
  (system / light / dark, in `localStorage`) into `data-theme` before first paint
  (no flash); Studio light lives on `:root`, Studio dark on `:root[data-theme="dark"]`,
  and a segmented toggle in the shell header switches it, with `system` following the
  OS live. Both palettes are the exact Studio token values.
- A **`Card`** surface component and a **tonal/ghost `Button`** variant were added so
  the shell realises the Studio "panels on warm porcelain" layout and uses the teal
  secondary role, not the magenta primary alone.
