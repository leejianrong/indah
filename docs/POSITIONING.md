...
| G5 | **Interactive maps** (pan/zoom, markers) | 13 | **Decide at review.** Static maps work today (render to an Image/Plot). An interactive map means bundling Leaflet (build-time, like uPlot) or shipping it as a custom component. Recommend a bundled client `Map` mirroring the `Chart` pattern. | pending (would extend the client-component approach) |
...
epic; the gap ADRs (0019–0021) are Proposed pending this review. **No demos get built
until the owner reviews this plan** (Phase 2 is planning only).
